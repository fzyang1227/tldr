...
export class Extension {
    private static autoState: ThemeState = 'light';
     ...
     */
    private static wasDarkTheme: boolean | null = null;
    ...
    private static async MV3syncSystemColorStateManager(isDark: boolean | null): Promise<void> {
        ...
        } else if (Extension.wasDarkTheme !== isDark) {
            Extension.wasDarkTheme = isDark;
    ...
    private static updateAutoState() {
        ...
        switch (mode) {
            ...
            case AutomationMode.SYSTEM:
                if (__CHROMIUM_MV3__) {
                    isAutoDark = Extension.wasDarkTheme;
                    if (Extension.wasDarkTheme === null) {
                        logWarn('System color scheme is unknown. Defaulting to Dark.');
                        isAutoDark = true;
                    }
                    break;
                }
                isAutoDark = Extension.wasDarkTheme === null
                    ? isSystemDarkModeEnabled()
                    : Extension.wasDarkTheme;
                if (isFirefox) {
    ...
    public static async collectData(): Promise<ExtensionData> {
        ...
        return {
            ...
            forcedScheme: Extension.autoState === 'dark' ? 'dark' : Extension.autoState === 'light' ? 'light' : null,
            activeTab,
        };
    }
    ...
    private static onColorSchemeChange = async (isDark: boolean) => {
        if (Extension.wasDarkTheme === isDark) {
